...
const iniComponents = async () => {
    try {
        ...
        const slideContainers = document.querySelectorAll('[data-slide-album]')
        const containers = document.querySelectorAll('[data-slide-album] .splide__list')
        
        containers.forEach((container, index) => {
            const limit = slideContainers[index].getAttribute('data-slide-album') || '' 
                            ...
                            <div class="p-4 !pb-0 flex flex-col gap-3 | lg:p-6">
                                <div class="flex items-center justify-between gap-4">
                                    <div class="inline-flex items-center gap-3" data-subtitle="">
                                        ...
                                        <div class="text-sm leading-none font-medium truncate">${item.album_date}</div>
                                    </div>
                                    <div class="text-sm leading-none font-medium">${item.album_total} Photos</div>
                                </div>
