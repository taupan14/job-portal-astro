...
const iniComponents = async () => {
    try {
        ...
        const slideContainers = document.querySelectorAll('[data-slide-testimony-avatar]')
        const containers = document.querySelectorAll('[data-slide-testimony-avatar] .splide__list')
        
        containers.forEach((container, index) => {
            const limit = slideContainers[index].getAttribute('data-slide-testimony-avatar') || '' 
            ...
            data.slice(0, itemsToDisplay).forEach(item => {
                ...
                const items = `
                    <div class="splide__slide w-auto !border-none">
                        <div class="relative isolate">
                            <div class="avatar avatar-lg outline-1 outline-transparent transition-all duration-800 ease-custom [.is-active-slide_&]:outline-offset-4 [.is-active-slide_&]:outline-primary-600">
                                ${initImgAvatar(item)} 
                            </div>
                            <div class="absolute inset-x-0 top-full py-4 flex flex-col items-center pointer-events-none">
                                <div class="whitespace-nowrap text-sm font-bold uppercase tracking-wide text-primary-600 translate-y-full opacity-0 transition-all duration-800 ease-custom [.is-active-slide_&]:translate-y-0 [.is-active-slide_&]:opacity-100">${item.name}</div>
                            </div>
                        </div>
                    </div>
                `
