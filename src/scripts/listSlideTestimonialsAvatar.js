...
const iniComponents = async () => {
  try {
    ...
    const el = document.querySelector("[data-slide-testimony-avatar]");
    let data = [];
    if (el) {
      data = JSON.parse(el.dataset.testimonials || "[]");
    }

    const slideContainers = document.querySelectorAll(
      "[data-slide-testimony-avatar]",
    );
    const containers = document.querySelectorAll(
      "[data-slide-testimony-avatar] .splide__list",
    );

    containers.forEach((container, index) => {
      const limit =
        slideContainers[index].getAttribute("data-slide-testimony-avatar") ||
        "";
